...
function indekseGoreFenomen(fenomenlerDizisi, i) {
  return i + ". indekste bulunan fenomen: " + fenomenler[i].profile;
}
...
function profilListesi(fenomenlerDizisi) {
  const profiller = [...fenomenlerDizisi];
  for (let i = 0; i < profiller.length; i++) {
    profiller[i] = profiller[i].profile;
  }
  return profiller;
}
...
function fenomenSil(fenomenlerDizisi, i) {
  const silme = [...fenomenlerDizisi];
  silme.splice(i, 1);
  return silme;
}
console.log(fenomenSil(fenomenler, 4));
/* Görev 6:
...
console.log(fenomenEkle(fenomenler, 6, "nida", 1000, 13, "instagram"));
/* Görev 7:
